...
export default defineComponent({
  ...
  mounted() {
    setInterval(this.currentDateUpdate, 1000)
  },
  ...
  methods: {
    ...
    handleBidButtonClick() {
      if (!this.address) {
        this.$emit('handleBidButtonClick', this.betValue)

        return
      }

      if (!this.isBetInputShown) {
        this.isBetInputShown = true

        return
      }

      if (!Number.isInteger(this.betValue)) {
        return
      }
  ...
  computed: {
    ...
    dateDifference() {
      ...
      const seconds = Math.floor(datesDifference / 1000)
      ...
      return timeArray.map(el => el.toString().padStart(2, "0")).join(":")
    },
  },
})
</script>
  ...
  <div v-else class="nft-auction-lot">
    <main>
      <div
          ...
          @click="handleImageClick(nftObject.nft.okxLink)"
      >
        <svg class="pointer" width="800px" height="800px" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
          <title>cursor-pointer-solid</title>
          <g id="Layer_2" data-name="Layer 2">
            <g id="invisible_box" data-name="invisible box">
              <rect width="48" height="48" fill="none"/>
            </g>
            <g id="Q3_icons" data-name="Q3 icons" fill="#1CC287">
    ...
    <div class="right">
      <div class="about">
        <div class="info">
          ...
          <div ref="descriptionWrapper" class="text">
            <h3 class="description" v-html="nftObject.nft.description" />

            <h4 class="button show-more" @click="$refs.descriptionWrapper.classList.toggle('active')">Show more</h4>
            <h4 class="button show-less" @click="$refs.descriptionWrapper.classList.toggle('active')">Show less</h4>
          </div>
        </div>

        <div class="other">
          <div class="winner" v-if="nftObject.status === 'Completed'">
          ...
          <aside>
            <div class="burned" v-if="nftObject.status === 'Completed'">
      ...
      >
        ...
        <h2 class="value">{{ dateDifference }}</h2>
      </div>
    </div>
  </div>
</template>
